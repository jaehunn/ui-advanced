...
function Pagination() {
  ...
  // page index 를 설정하자.
  this.currentPageIndex = 1; // 5개의 페이지에 걸쳐서 보여줄 것이다.
  // 자연스럽게 데이터 개수 / 페이지 당 보여줄 데이터 개수로 페이지 개수가 도출된다.

  this.itemsPerPage = 5; // 소숫점(페이지 당 보여줄 데이터를 채우지 못한 나머지 데이터 개수) 을 살려서 페이지 하나로 가져가야한다. -> 올림처리
...
Pagination.prototype.renderItem = function (items) {
  return items.reduce(function (result, _ref) {
    var profile_url = _ref.profile_url,
        author = _ref.author,
        content = _ref.content,
        createdAt = _ref.createdAt;
    return result + "\n        <div class=\"item\">\n          <img src=\"".concat(profile_url, "\" alt=\"\" />\n          <span class=\"author\">").concat(author, "</span>\n          <div class=\"createdAt\">").concat(createdAt, "</div>\n          <div class=\"content\">").concat(content, "</div>\n        </div>\n        ");
  }, "");
};

Pagination.prototype.getItemsByPageIndex = function (pageIndex) {
  var startItemIndex = (pageIndex - 1) * this.itemsPerPage;
  var endItemIndex = pageIndex * this.itemsPerPage; // 바뀌기 전 인덱스로 범위잡고 업데이트한다.

  this.currentPageIndex = pageIndex; // [0. ...index)
...
Pagination.prototype.renderPageButton = function () {
  ...
  return Array(this.itemsPerPage).fill().reduce(function (result, _, index) {
    // active 된 버튼은 현재 페이지 인덱스 멤버를 참조하면 되겠다.
    var className = index + 1 === _this.currentPageIndex ? "active" : "";
    return result + "<button class=\"".concat(className, "\">").concat(index + 1, "</button>");
  }, "");
};

document.addEventListener("DOMContentLoaded", function () {
  var pagination = new Pagination(); // 요소 타게팅
  // 1. item 들을 감싸는 wrapper -> item 들을 렌더링
  // 2. page button -> 클릭 이벤트

  var $itemContainer = document.querySelector(".item-container");
  var $pageContainer = document.querySelector(".page-container"); // 버블링
  // 마운트때 데이터를 렌더링 시켜야한다.
  // 첫 페이지에 대해 렌더링하자.

  var items = pagination.getItemsByPageIndex(1);
  $itemContainer.innerHTML = pagination.renderItem(items); // 버튼은 고정적으로 렌더링된다.

  $pageContainer.innerHTML = pagination.renderPageButton(); // 버튼 클릭이벤트 등록

  $pageContainer.addEventListener("click", function (_ref2) {
    var target = _ref2.target;
    var targetPageIndex = target.innerText;
    var items = pagination.getItemsByPageIndex(+targetPageIndex);
    $itemContainer.innerHTML = pagination.renderItem(items); // 멤버에 접근할 수 없다.
