...
const itemsSlice = createSlice({
  ...
  reducers: {
    ...
    getCurrentPageItems(state, action) {
      const { items, itemsPerPage } = state;
      const {
        payload: { targetPageNum },
      } = action;

      const startIndexOfCurrentPageItemList = (targetPageNum - 1) * itemsPerPage;
      const endIndexOfCurrentPageItemList = targetPageNum * itemsPerPage;

      // 데이터를 가공해서 적재하고
      state.currentPageItems = items.slice(startIndexOfCurrentPageItemList, endIndexOfCurrentPageItemList);
...
export const { getCurrentPageItems } = itemsSlice.actions;
