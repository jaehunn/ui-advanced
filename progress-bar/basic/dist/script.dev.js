...
(function App() {
  ...
  function ProgressBar($element) {
    ...
    this.widthPerIndex = 100 / (this.lastIndex - this.currentIndex); // this.intervalTarget = null 멤버로 등록해서 중복되는 interval 을 제어할 수 없다 어떻게 해결할까?
  ...
  ProgressBar.prototype.movePrev = function () {
    ...
    var intervalTarget = setInterval(function () {
      // 콜백에서 범위를 체크하고,
  ...
  ProgressBar.prototype.moveNext = function () {
    ...
    this.currentIndex += 1;
    var nextWidth = this.currentIndex * this.widthPerIndex;
    var intervarTarget = setInterval(function () {
      if (currentWidth <= nextWidth) {
        currentWidth += 1;
